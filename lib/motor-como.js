...
export async function motorComoRag({ pregunta, ley, pais = 'MX' }) {
  ...
  if (grupos.length === 0) {
    ...
    if (porTitulo.length === 0) {
      ...
      console.error('DBG_A ley=' + ley + ' pais=' + pais)
      console.error('DBG_B queries=' + JSON.stringify(queries))
      console.error('DBG_C capRows=' + capFlat.length + ' titRows=' + titFlat.length)
      console.error('DBG_D capFirst=' + JSON.stringify(capFlat[0] || null))
      console.error('DBG_E titFirst=' + JSON.stringify(titFlat[0] || null))

...
async function rpcWithRetry(fn, retries = 3) {
  for (let i = 0; i < retries; i++) {
    ...
    console.error('[motor-como] RPC error:', r.error.message || r.error.code)
    return []
  }
}
...
CRITICAL — VISUALIZATION FORMAT: since you are explaining a procedure, include ONE Mermaid flowchart. It MUST be wrapped in an HTML <div class="mermaid"> tag — without this wrapper, the diagram will NOT render and will appear as broken text to the user.
...
FORBIDDEN FORMATS:
- Raw "flowchart TD ..." without the <div> wrapper — DO NOT do this.
- Markdown fences like \`\`\`mermaid ... \`\`\` — DO NOT do this.
- Any other tag instead of <div class="mermaid">.

Put each node definition and each arrow on its own line inside the <div>. Supported diagram types: flowchart, sequenceDiagram, stateDiagram-v2, timeline. Only ONE diagram per response. Skip the diagram entirely if the question is merely definitional (single concept, no process).

SIZE LIMIT: the diagram must have AT MOST 10 nodes and AT MOST 12 edges. Keep each node label under 60 characters — use short clauses, not long citations. Put citations in the prose, not in node labels. The goal is a readable high-level flow, not a reproduction of the entire procedure.`
}
...
export async function motorComoRag({ pregunta, ley, pais = 'MX' }) {
  ...
  if (grupos.length === 0) {
    ...
    if (porTitulo.length === 0) {
      return { ok: false, error: 'No se encontró estructura jerárquica para esta consulta en la ley seleccionada.' }
    }
...
export async function motorComo({ pregunta, ley, pais = 'MX' }) {
  ...
  const flujo = await geminiConFallback(
    { model: 'gemini-2.5-flash', contents: sintesisPrompt,
      config: { thinkingConfig: { thinkingBudget: 0 }, temperature: 0.2, maxOutputTokens: 4500 } },
    sintesisPrompt,
    { maxTokens: 2000 }
  )

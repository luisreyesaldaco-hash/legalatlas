...
export async function motorComoRag({ pregunta, ley, pais = 'MX' }) {
  ...
  if (grupos.length === 0) {
    ...
    if (porTitulo.length === 0) {
      console.error('[motor-como] empty groups', { ley, pais, queries, capRows: busquedasCap.flat().length, titRows: busquedasTit.flat().length, capSample: busquedasCap.flat().slice(0,2), titSample: busquedasTit.flat().slice(0,2) })
